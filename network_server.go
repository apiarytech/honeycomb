...
func StartServer(db *TagDatabase, validTokens []string, port, certFile, keyFile string, serverReady *sync.WaitGroup, ctx context.Context) {
	...
	// 5. Start the server in a new goroutine so it doesn't block the main application.
	go func() {
		log.Printf("[Server] Starting honeycomb network server on port %s...", port)
		// Signal that the server setup is complete and it's about to start listening.
		if serverReady != nil {
			serverReady.Done()
		}
	...
	// 6. Listen for the context to be canceled to trigger a graceful shutdown.
	go func() {
		<-ctx.Done()
